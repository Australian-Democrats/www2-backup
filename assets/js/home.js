...
(function(){
  ...
  if(!join&&!don)return;
  var emailRe=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  // Same Australian-mobile rule as /join's mobileValid() and the server's asAuMobile():
  // 04XXXXXXXX, 4XXXXXXXX, or +61 4XXXXXXXX with any spacing. Checked on leaving the
  // field (so the box goes red before submit) and again at submit; the server still
  // refuses anything else regardless.
  function mobileOk(v){var d=(v||'').replace(/\s+/g,'').replace(/^\+?61/,'').replace(/\D/g,'');return (d.length===9&&d.charAt(0)==='4')||(d.length===10&&d.slice(0,2)==='04');}
  function mobileDigits(v){return (v||'').replace(/\D/g,'').replace(/^0/,'');}
  var MOBILE_MSG='Enter a valid Australian mobile, e.g. 0412 345 678.';
  ...
  if(join){var tier='supporter',roll='',party='',silent='no_unsure',noEmail=false;
    ...
    $('#hjMobile').addEventListener('blur',function(){if(this.value.trim()&&!mobileOk(this.value))inval(this);});
    join.addEventListener('submit',function(e){e.preventDefault();var msg=$('#hjMsg');clearInval(join);var bad=null;
      ...
      var mb=$('#hjMobile');if(!mobileOk(mb.value)){inval(mb);if(!bad)bad=mb;}
      ...
      if(bad){msg.style.color='#b0333a';msg.textContent=(bad===mb&&mb.value.trim())?MOBILE_MSG:'Please complete the highlighted fields.';(bad.scrollIntoView?bad:con).scrollIntoView({behavior:'smooth',block:'center'});if(bad.focus)try{bad.focus();}catch(e){}return;}
      go(join,{formType:'join',tier:tier,autoRenew:tier==='supporter'?false:$('#hjRecur').checked,firstName:$('#hjFirst').value.trim(),middleName:$('#hjMiddle').value.trim(),lastName:$('#hjLast').value.trim(),preferredName:$('#hjPreferred').value.trim(),email:noEmail?'':em.value.trim(),noEmail:noEmail,mobile:mobileDigits(mb.value),landline:(function(){var l=$('#hjLandline');var a=$('#hjLandlineArea');return (l&&l.value.trim())?((a&&a.value||'')+' '+l.value.trim()).trim():'';})(),dob:[dD.value,dM.value,dY.value].join('/'),residential:{line1:$('#hjStreet').value.trim(),line2:$('#hjStreet2').value.trim(),city:$('#hjCity').value.trim(),postcode:pc.value.trim(),state:st.value,country:'AU'},onElectoralRoll:roll,silentElector:silent,otherParty:{member:party==='yes',name:$('#hjOtherParty').value.trim()},consent:true,campaign:AD.qp('campaign'),turnstileToken:token(join)},msg,$('#hjSubmit'));
    });
  }
  if(don){var amt=50,freq='once',cust=$('#hdCustom');
    ...
    var hdMob=$('#hdMobile');
    if(hdMob)hdMob.addEventListener('blur',function(){if(this.value.trim()&&!mobileOk(this.value))inval(this);});
    don.addEventListener('submit',function(e){e.preventDefault();var msg=$('#hdMsg');clearInval(don);var bad=null;
      ...
      // Mobile is optional here, but if given it must be an AU mobile — the server rejects anything else.
      if(hdMob&&hdMob.value.trim()&&!mobileOk(hdMob.value)){inval(hdMob);if(!bad)bad=hdMob;}
      ...
      if(bad){msg.style.color='#b0333a';msg.textContent=(bad===hdMob)?MOBILE_MSG:'Please complete the highlighted fields.';bad.scrollIntoView({behavior:'smooth',block:'center'});if(bad.focus)try{bad.focus();}catch(e){}return;}
      // The donor's optional mobile was collected but never sent before 2026-09-18.
      go(don,{formType:'donate',amount:v,frequency:freq,firstName:$('#hdFirst').value.trim(),lastName:$('#hdLast').value.trim(),email:em.value.trim(),mobile:(hdMob&&hdMob.value.trim())?mobileDigits(hdMob.value):'',address:{line1:$('#hdAddr1').value.trim(),line2:$('#hdAddr2').value.trim(),city:$('#hdCity').value.trim(),state:st.value,postcode:pc.value.trim(),country:'AU'},eligibilityConfirmed:true,emailConsent:true,alsoJoin:$('#hdAlsoJoin').checked,campaign:AD.qp('campaign'),turnstileToken:token(don)},msg,$('#hdSubmit'));
    });
  }
})();
